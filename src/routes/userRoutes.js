...
userRouter.route('/user/login').post(async (req, res) => {
  try {
    // Chama a função de login do userControl
    const result = await userControl.loginUser(req, res);

    // Exibe as informações no console
    console.log(result.message);

    // Retorna a resposta ao cliente (pode ser uma mensagem de sucesso, token, etc.)
    // res.status(200).json(result);
  } catch (error) {
    // Se houver um erro, retorne um status de erro e a mensagem apropriada
