...
exports.createUser = async (req, res) => {
  ...
  if (!name) {
    return res.status(400).json({ message: 'Preencha seu Nome!' });
  }
  try {
    ...
    return res.status(201).json({
      message: `Registro concluído, seu token é:${geradorToken({ id: num })}`,
    });
  } catch (error) {
...
exports.loginUser = async (req, res) => {
  ...
  if (!password) {
    return { message: 'A senha está nula, por favor digite algo válido' };
  }
  const userQuery = await User.findOne({ name }).select('password');
  if (!userQuery) {
    return res.status(400).json({ message: 'Usuário não localizado' });
  }
  if (!(await bcrypt.compare(password, userQuery.password))) {
  ...
  return res
    .status(201)
    .json({
      message: `login concluído, seu token é:${geradorToken({ id: id })}`,
    });
};
...
exports.getUserById = async (req, res) => {
  try {
    ...
    if (!userName) {
      return res
        .status(406)
        .json({ message: 'Digite o nome para que a consulta seja realizada!' });
    }
    const findUser = await User.findOne({ name: userName }).select([
      '-__v',
      '-password',
    ]);
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const userBd = await User.findOne({ name: name }).select(['password']);
    if (!userBd) {
    ...
    await User.findOneAndUpdate({ name: name }, dataUser, { new: true });
...
exports.productBuy = async (req, res) => {
  try {
    const findTrolley = await Carrinho.findOne({
      user: req.userId.id,
    }).populate('products.identifier');

    if (!findTrolley || findTrolley.products.length <= 0) {
      return res
        .status(500)
        .json({ message: 'Erro na compra, pois seu carrinho está vázio' });
    }
    let total = 0;
    ...
    await Invoice.create({
      user: req.userId.id,
      products: findTrolley.products,
      valueTotal: total.toFixed(2),
    });
  ...
  try {
    if (!req.params.id) {
      return res.status(500).json({
        message:
          'Faltam dados referente ao produto, para que seja realizado a busca',
      });
    }
    if (!req.body.amount) {
      return res
        .status(500)
        .json({ message: 'É necessário preencher o campo quantidade' });
    }
...
exports.removeProduct = async (req, res) => {
  try {
    ...
    if (!idCart) {
      return res
        .status(500)
        .json({ message: 'Forneça o id do produto, para ser removido' });
    }
    const findTrolley = await Carrinho.findOne({
      user: req.userId.id,
    }).populate('products.identifier');
    if (!findTrolley || findTrolley.products.length <= 0) {
      return res
        .status(500)
        .json({ message: 'Não foi encontrado compras em seu nome' });
    }
    const productToUpdate = findTrolley.products.find(
      (x) => productId === x.identifier._id.toHexString(),
    );
    if (!productToUpdate) {
